...
# Z1_total/Z2_total/objective_total — via _accumulate_weighted_objective) and
# folds wobj_Z1_total/wobj_Z2_total/wobj_objective_total/alpha/beta/weights/rho
# plus total_weighted_delay/offset_correction_magnitude aliases into the run's
# metrics dict, which append_master_csv() then writes to batch_results.csv.
...
EXPERIMENTS = [
    ...
    # sensitive to the actual pax·s saved vs car delay cost.
    {
        ...
        "reward_overrides": {
            ...
            "DCTSP_W_H":             0.45,   # tuned: reduced from 0.6 → more CPD weight
            "DCTSP_CAR_WEIGHT":      0.35,   # tuned: slightly lower car cost weight
        },
    },
    # ── MARL + Harmony Search ──────────────────────────────────────────────────
...
# Sweeps the threshold at which bus lateness "takes over" and priority is
# granted unconditionally, regardless of cross-traffic cost or saturation.
#
#   META_LATENESS_TAKEOVER_S — bus headway-deviation (seconds) that triggers
#   unconditional TSP.  When |σ_in| ≥ this value, both the minimum-delay gate
#   (META_MIN_BUS_DELAY_S) and the saturation-avoidance gate (META_BALANCE_FACTOR)
#   are bypassed.
#
#   Values:
#     0    = always-grant for ANY positive lateness (extreme sensitivity)
#     30   = grant when bus is >30s late
#     60   = grant when bus is >1min late
#     120  = grant when bus is >2min late
#     9999 = disabled (never takeover — normal gating applies)
#
# Crossed with META_HW_SENSITIVITY to show interaction of soft (adaptive weight)
# and hard (takeover) lateness mechanisms.
LATENESS_SWEEP_ENABLED              = False
LATENESS_SWEEP_TAKEOVER             = [0, 30, 60, 120, 9999]
LATENESS_SWEEP_HW_SENSITIVITY       = [0.5, 1.0, 1.5, 2.5]
    ...
    for _lhw in LATENESS_SWEEP_HW_SENSITIVITY:
        EXPERIMENTS.append({
            ...
            "reward_overrides": {
                ...
                "META_MIN_BUS_DELAY_S":      0,   # open all regular gates
                "META_BALANCE_FACTOR":        10.0,  # open saturation gate
            },
        })
...
def write_run_config(experiment_name, strategy, seed, scalar,
    ...
    log(f"run_config written: experiment={experiment_name} "
        ...
        f"coord_algo={coordination_algo} global_reward={global_reward_mode} "
...
def collect_run_metrics(project_dir, strategy, seed, scalar,
    ...
    # ── 7. Weighted-objective (Z1/Z2 composite) trace — totals for this run ──
    ...
    # plot total_weighted_delay (Z1) vs offset_correction_magnitude (Z2) —
    # i.e. find the Pareto frontier across an ALPHA/BETA grid.
        ...
        try:
            ...
            if _wobj_rows:
                ...
                log(f"  Weighted objective totals: Z1={meta['wobj_Z1_total']:.1f} "
                    f"Z2={meta['wobj_Z2_total']:.1f} obj={meta['wobj_objective_total']:.1f} "
                    f"(alpha={meta['wobj_alpha']} beta={meta['wobj_beta']}, n={meta['wobj_n']})")
...
def write_core_output(core_path, metrics, reward_overrides):
    ...
    row = {
        ...
        "strategy_mode": (
            "ZIG" if _overrides.get("DCTSP_ZIG_MODE")
...
def main():
                ...
                if exp_name == "DCTSP_BARGAIN_SPM":
                    try:
                        ...
                        if _os.path.isfile(_dash_script):
                            ...
                            # Call main() directly — avoids argparse/SystemExit at module level
                            _dmod.main()
    ...
    # ── Render Quarto sensitivity report ─────────────────────────────────────
