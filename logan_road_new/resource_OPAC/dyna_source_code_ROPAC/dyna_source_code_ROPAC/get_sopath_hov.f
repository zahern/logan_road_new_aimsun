       subroutine get_sopath_hov(j,i,iselect,currenttime)
...
!        if(soaccuprob_hov(ifrom,ito,timeindex,np).ge.uniform) exit
!       enddo
!	 ipick = np 
	 ipick = jipick(j)	

!       traverse=>MucPath_hov(ifrom,ito,
!     +           sopath_hov(ifrom,ito,timeindex,ipick))
!
...
!       enddo
	...
	do ihh =1,MucPathAtt_Hov(ifrom,ito, 
     +           sopath_Hov(ifrom,ito,timeindex,ipick))%node_number

	  valso= MUCPath_Hov_Array(ifrom,ito,
     +           sopath_Hov(ifrom,ito,timeindex,ipick))%P(ihh)
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
       ...
       subroutine get_genelink_from_sopath_hov(j,igenelink)
	 ...
	 use vector_mod
       ...
       do np = 1, NumSopath_hov(ifrom,ito,timeindex)
        if(soaccuprob_hov(ifrom,ito,timeindex,np).ge.uniform) exit
       enddo
	 jipick(j) = np 
	...
	isize= MucPathAtt_hov(ifrom,ito,
     +           sopath_Hov(ifrom,ito,timeindex,jipick(j)))%node_number

	  iupstreamnode= MUCPath_Hov_Array(ifrom,ito,
     +           sopath_Hov(ifrom,ito,timeindex,jipick(j)))%P(2)

	  idownstreamnode= MUCPath_Hov_Array(ifrom,ito,
     +           sopath_Hov(ifrom,ito,timeindex,jipick(j)))%P(3)
